...
export const BotaoWhatsApp: React.FC<BotaoWhatsAppProps> = ({
  ...
  const abrirWhatsApp = async () => {
    try {
      // 1. Registrar que conversa foi iniciada usando SQL direto
      const { error } = await supabase
        .from('conversas_whatsapp_log')
        .insert({
          reserva_id: reservaId,
          usuario_iniciou: (await supabase.auth.getUser()).data.user?.id,
          usuario_recebeu: usuarioRecebeuId,
          item_id: reservaId, // Temporário - seria melhor buscar o item_id da reserva
