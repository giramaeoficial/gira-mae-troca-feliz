...
export const usePublicarItemFormV2 = () => {
 ...
 const handleSubmit = async (e: React.FormEvent) => {
   ...
   try {
     ...
     // Determinar status baseado no cadastro
     const itemStatus = profile?.cadastro_status === 'completo' ? 'disponivel' : 'inativo';
     ...
     publicarItem(
       { itemData, fotos: formData.imagens },
       {
         onSuccess: () => {
           ...
           if (itemStatus === 'inativo') {
             toast.success("Item salvo! Complete seu cadastro para ativá-lo.");
             navigate('/cadastro/address'); // ou próximo step
           } else {
