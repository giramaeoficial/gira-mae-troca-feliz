...
const PublicarItem = () => {
  ...
  const isMissao = location.pathname.includes('primeiro-item') || 
                   location.pathname.includes('missao') ||
                   location.state?.isMissao;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isMissao) {
      // Para missão, aguardar o submit
      await originalSubmit(e);
      
      // Se é o primeiro item e não teve erro, avançar para o segundo
      if (currentItem === 1 && !loading) {
        setCurrentItem(2);
        // Reset form para segundo item
        updateFormData({
          titulo: '',
          descricao: '',
          categoria_id: '',
